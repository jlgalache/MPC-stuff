  ...
  for line in data_file:
    if line.strip():
      ...
      if 'days' in arc:
        ...
        arc_length = ''
        arc_years = arc
      output_json.append({'number':number,'Name':name,'Designation':desig,'Other_desigs':other_desig,'H':H,'G':G,'Epoch':round(epoch,7),'M':float(line[26:35].strip()),'Peri':float(line[37:46].strip()),'Node':float(line[48:57].strip()),'i':float(line[59:68].strip()),'e':float(line[70:79].strip()),'n':float(line[80:91].strip()),'a':float(line[92:103].strip()),'U':U,'ref':line[107:116].strip(),'Num_obs':num_obs,'Num_opps':int(line[123:126].strip()),'Arc_length':arc_length,'Arc_years':arc_years,'rms':rms,'Perturbers':line[142:145].strip(),'Perturbers_2':line[146:149].strip(),'Computer':line[150:160].strip(),'Hex_flags':line[161:165].strip(),'Last_obs':last_obs,'Tp':float(line[203:216].strip()),'Orbital_period':round(period,7),'Perihelion_dist':round(q_small,7),'Aphelion_dist':round(Q_big,7),'Semilatus_rectum':round(semilatus,7)})
